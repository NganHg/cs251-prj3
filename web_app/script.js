...
async function getNeighbors(node) {
	...
	for (var j = 0; j < users.length; j++){
		debt = await BlockchainSplitwise.lookup(node, users[j]);
		if (debt > 0) neighbors.push(users[j]);
	}
	return neighbors;
}
...
async function getUsers() {
	function_calls = await getAllFunctionCalls(contractAddress, "add_IOU");
	const users = new Set();
	for (var j = 0; j < function_calls.length; j++){
		function_call = function_calls[j];
		users.add(function_call.args[0]);
		users.add(function_call.args[1]);
	}
...
async function getTotalOwed(user) {
	toltalOwed = await BlockchainSplitwise.getTotalOwed(user);
	return toltalOwed;
}
...
async function getLastActive(user) {
	function_calls = await getAllFunctionCalls(contractAddress, "add_IOU");
	for (var j = 0; j < function_calls.length; j++){
		func_call = function_calls[j];
		if ((user.toLowerCase() == func_call.args[0].toLowerCase()) || (user.toLowerCase() == func_call.args[1].toLowerCase())) {
			return func_call.t;
		}
	}
	return null;
...
async function add_IOU(creditor, amount) {
	...
	console.log(path);
	if (path != null) {
		min = await BlockchainSplitwise.lookup(defaultAccount, creditor);
		for (var j = 0; j < path.length - 1; j++){
			amount = await BlockchainSplitwise.lookup(path[j], path[j+1]);
			if (min > amount) min = amount;
		}
		tx = await BlockchainSplitwise.reduceIOU(path, min);
		tx.wait();
	}
}
